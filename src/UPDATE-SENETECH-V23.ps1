...
$StartupRoot = Join-Path $env:ProgramData 'SENETECH'
$StartupBeginPath = Join-Path $StartupRoot 'startup.begin'
$StartupOkPath = Join-Path $StartupRoot 'startup.ok'
$StartupErrorPath = Join-Path $StartupRoot 'startup.error'
...
function Add-CacheBuster([string]$Url) {
    ...
    if ($Url.Contains('?')) { return ($Url + '&senetech=' + $token) }
    return ($Url + '?senetech=' + $token)
}
...
function Copy-SenetechRuntime([string]$Source,[string]$Destination) {
    ...
    foreach ($name in @('SENETECH-Setup.exe','VERSION.txt','LISEZ-MOI.txt','CHANGELOG.txt','UPDATE-SENETECH.ps1','UPDATE-SENETECH-CHANNEL.ps1')) {
        ...
        if (Test-Path -LiteralPath $src) { Copy-Item -LiteralPath $src -Destination (Join-Path $Destination $name) -Force -ErrorAction Stop }
    }
...
function Invoke-DevelopmentPatch([string]$PatchUrl,[string]$ExpectedHash = '') {
    ...
    $requestUrl = Add-CacheBuster $PatchUrl
    Invoke-WebRequest -Uri $requestUrl -Headers $headers -OutFile $patchPath -UseBasicParsing -TimeoutSec 30

    if (-not [string]::IsNullOrWhiteSpace($ExpectedHash)) {
        $actual = Get-Sha256 $patchPath
        if ($actual -ne $ExpectedHash.ToLowerInvariant()) { throw "Patch SHA-256 mismatch: $actual" }
        Write-UpdateLog 'Patch SHA-256 verification OK.'
    }
    ...
    if ($patchProcess.ExitCode -ne 0) {
        $detail = @($errorLines | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) } | Select-Object -Last 6) -join ' | '
...
function Test-PowerShellSyntax([string]$Root) {
    ...
    foreach ($file in $files) {
        ...
        if ($errors -and $errors.Count -gt 0) {
            $first = $errors[0]
            throw ("PowerShell syntax error in {0}: {1}" -f $file.FullName,$first.Message)
        }
    }
    Write-UpdateLog ("PowerShell syntax preflight OK: {0} script(s)." -f $files.Count)
}

Initialize-UpdateLog
Write-UpdateLog "Updater V23 start. Current=$CurrentVersion InstallDir=$InstallDir PID=$WaitForProcessId"

try {
    try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 } catch { }
    $headers = @{
        'User-Agent' = "SENETECH-Setup/$CurrentVersion"
        'Cache-Control' = 'no-cache'
        'Pragma' = 'no-cache'
    }

    $manifestRequestUrl = Add-CacheBuster $ManifestUrl
    Write-UpdateLog "Checking $ManifestUrl (cache bypass)"
    $manifest = Invoke-RestMethod -Uri $manifestRequestUrl -Headers $headers -UseBasicParsing -TimeoutSec 30
    ...
    if ($UpdateChannel -eq 'develop' -and
        ...
        -not [string]::IsNullOrWhiteSpace([string]$manifest.incrementalPatchUrl) -and
        $local.Major -eq 1 -and $local.Minor -eq 6 -and $local.Build -eq 0 -and $local.Revision -ge 17) {
    ...
    if ($incrementalMode) {
        ...
        $patchHash = ''
        if ($manifest.PSObject.Properties.Name -contains 'incrementalPatchSha256') { $patchHash = [string]$manifest.incrementalPatchSha256 }
        Invoke-DevelopmentPatch ([string]$manifest.incrementalPatchUrl) $patchHash
    } else {
        ...
        Invoke-WebRequest -Uri $baseUrl -Headers $headers -OutFile $ZipPath -UseBasicParsing -TimeoutSec 120
        if ($baseSize -gt 0) {
        ...
        if ($manifest.PSObject.Properties.Name -contains 'patchUrl' -and $manifest.patchUrl) {
            $patchHash = ''
            if ($manifest.PSObject.Properties.Name -contains 'patchSha256') { $patchHash = [string]$manifest.patchSha256 }
            Invoke-DevelopmentPatch ([string]$manifest.patchUrl) $patchHash
        }
    }
    ...
    if ($manifest.PSObject.Properties.Name -contains 'overlayFiles' -and $manifest.overlayFiles) {
        foreach ($overlay in @($manifest.overlayFiles)) {
            ...
            Invoke-WebRequest -Uri (Add-CacheBuster $url) -Headers $headers -OutFile $target -UseBasicParsing -TimeoutSec 45
    ...
    Write-UpdateLog 'Staged package file validation OK.'
    Test-PowerShellSyntax $StageDir
    ...
    $qBegin = Quote-PsLiteral $StartupBeginPath
    $qOk = Quote-PsLiteral $StartupOkPath
    $qError = Quote-PsLiteral $StartupErrorPath
...
`$StartupBeginPath = $qBegin
`$StartupOkPath = $qOk
`$StartupErrorPath = $qError
...
function Reset-ManagedRuntime([string]`$Directory) {
    foreach (`$name in @('SENETECH-Setup.exe','VERSION.txt','LISEZ-MOI.txt','CHANGELOG.txt','UPDATE-SENETECH.ps1','UPDATE-SENETECH-CHANNEL.ps1')) {
        Remove-Item -LiteralPath (Join-Path `$Directory `$name) -Force -ErrorAction SilentlyContinue
    }
    `$engineDir = Join-Path `$Directory '_SENETECH'
    if (Test-Path -LiteralPath `$engineDir) { Remove-Item -LiteralPath `$engineDir -Recurse -Force -ErrorAction Stop }
}
...
function Restore-PreviousRuntime {
    ...
    Reset-ManagedRuntime `$InstallDir
    Copy-Tree `$BackupDir `$InstallDir
    Log 'Rollback copy completed.'
    if (-not `$NoRestart) {
        try { `$old = Restart-Runtime `$InstallDir; Log ("Previous runtime relaunched. PID=" + `$old.Id) }
        catch { Log ("Previous runtime relaunch failed: " + `$_.Exception.Message) }
    }
}
...
try {
    ...
    Log 'New runtime copied cleanly.'
    ...
    if (-not `$NoRestart) {
        Remove-Item -LiteralPath `$StartupBeginPath,`$StartupOkPath,`$StartupErrorPath -Force -ErrorAction SilentlyContinue
        `$newProcess = Restart-Runtime `$InstallDir
        Log ("Restart requested. Launcher PID=" + `$newProcess.Id)
        ...
        if (`$StartupVerification) {
            ...
            while ((Get-Date) -lt `$deadline -and -not `$verified) {
                if (Test-Path -LiteralPath `$StartupErrorPath) {
                    `$startupError = Get-Content -LiteralPath `$StartupErrorPath -Raw -ErrorAction SilentlyContinue
                    if (`$startupError) { throw ("Startup error: " + `$startupError) }
                }
                if (Test-Path -LiteralPath `$StartupOkPath) {
                    `$marker = Get-Content -LiteralPath `$StartupOkPath -Raw -ErrorAction SilentlyContinue
                    if (`$marker -and `$marker.Contains("version=`$RemoteVersion")) {
                        `$verified = `$true
                        Log ("Startup marker OK: " + `$marker.Trim())
                        break
                    }
                }
                if (`$newProcess.HasExited -and -not `$launcherExitLogged) {
                    `$launcherExitLogged = `$true
                    try { Log ("Launcher exited while waiting for runtime marker. ExitCode=" + `$newProcess.ExitCode) } catch { Log 'Launcher exited while waiting for runtime marker.' }
                }
                Start-Sleep -Milliseconds 500
            }
            if (-not `$verified) {
                `$begin = ''
                if (Test-Path -LiteralPath `$StartupBeginPath) { `$begin = Get-Content -LiteralPath `$StartupBeginPath -Raw -ErrorAction SilentlyContinue }
                throw ("Startup verification timeout for SENETECH `$RemoteVersion. BeginMarker=" + `$begin)
            }
        } else {
            Start-Sleep -Seconds 4
            Log 'Startup verification disabled by manifest for this migration build.'
        }
    }
    ...
    try {
        ...
        [System.Windows.MessageBox]::Show(
            "La mise a jour SENETECH a echoue et a ete annulee.`r`n`r`n" + `$_.Exception.Message + "`r`n`r`nJournal :`r`n" + `$PersistentLogPath,
